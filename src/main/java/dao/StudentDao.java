...
public class StudentDao extends Dao {
	...
	public Student get(String no) throws Exception {
		...
		try {
			...
			statement.setString(1, no); //上に書いた?マーク１個目に変数noの情報をセット
			
			// プリペアードステートメントを実行
			ResultSet resultSet=statement.executeQuery();

			// 学校Daoを初期化 studentインスタンスに学校情報を入れるために必要
			SchoolDao schoolDao=new SchoolDao();
			

			// ResultSet resultSet = null; // エラー解消のため宣言　書き換え必要
			if (resultSet.next()) {
				// リザルトセットが存在する場合
				// 学生インスタンスに検索結果をセット
				// resultSet.getデータ型("テーブルの項目名")で結果を取得
